        ...
        <div class="table-body-wrap">
            <div style="float:left">
                <table border="2" class="stripe" style="width: auto!important;border-right: none;">
                    ...
                    <tr>
                        <td colspan="2" class="alignCenter" style="background-color: #fff;height: 63.75px;">症状</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="alignCenter">足背动脉搏动</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="alignCenter">主食（克/天）</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="alignCenter" rowspan="2">辅助检查</td>
                        <td class="alignCenter">空腹血糖值</td>
                    </tr>
                    <tr>
                        <td class="alignCenter" style="height: 64.75px;">其他检查</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td colspan="2" class="alignCenter">低血糖反应</td>
                    </tr>
                    <tr>
                        <td colspan="2" class="alignCenter">此次随访分类</td>
                    </tr>
                    <tr>
                        <td class="alignCenter" rowspan="2" :style="{ height: 88.98 * (maxNum + 1) + 'px' }">用药情况</td>
                        <td class="alignCenter" style="height: 88.98px;">胰岛素</td>
                    </tr>
                    <!-- <tr>
                        <td class="alignCenter" style="height:65px;">用法用量</td>
                    </tr> -->
                    <tr>
                        <td class="alignCenter" :style="{ height: 88.98 * maxNum + 'px' }">其他</td>
                    </tr>
                    <tr>
            ...
            <TableDrag width="800" style="border-right: 2px solid #999999;">
                <table border="2" class="stripe" style="width: 810px!important;border-left: none;border-right: none;cursor: pointer;">
                    ...
                    <tr>
                        <td class="mark" v-for="(item, index) in visitList">{{ $formatDate(item.thisFlupDate) }}&nbsp;</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="mark" v-for="(item, index) in visitList">
                            <span v-if="item.footDorsalArteryPulseFlag == '1'">触及正常</span>
                            <span v-if="item.footDorsalArteryPulseFlag == '2'">减弱</span>
                            <span v-if="item.footDorsalArteryPulseFlag == '3'">消失</span>&nbsp;
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="mark" v-for="(item, index) in visitList">{{ item.drinkingDailyAmount }}&nbsp;</td>
                    </tr>
                    <tr>
                        <td v-for="(item, index) in visitList">
                            <span class="lineBox alignCenter">{{ item.motionFrequencyCode }}</span
                            >次/周 <span class="lineBox alignCenter">{{ item.motionDuration }}</span
                            >分钟/次
                        </td>
                    </tr>
                    <tr>
                        <td class="mark" v-for="(item, index) in visitList">{{ item.dailyFoodIntake }}&nbsp;</td>
                    </tr>
                    <tr>
                        <td class="mark" v-for="(item, index) in visitList">
                            <span v-if="item.psychicAdjustIcValueCode == '1'">良好</span>
                            <span v-if="item.psychicAdjustIcValueCode == '2'">一般</span>
                            <span v-if="item.psychicAdjustIcValueCode == '3'">差</span>&nbsp;
                        </td>
                    </tr>
                    <tr>
                        <td v-for="(item, index) in visitList" class="noOver">
                            <span class="mark" :title="item.flUpComplianceIcValueName">{{ item.flUpComplianceIcValueName }}&nbsp;</span>
                        </td>
                    </tr>
                    <tr>
                        <td v-for="(item, index) in visitList">
                            <span class="mark">{{ item.fastingBloodValue }}&nbsp;</span>
                        </td>
                    </tr>
                    <tr>
                        <td v-for="(item, index) in visitList" style="text-align: left;">
                            糖化血红蛋白<span class="lineBox alignCenter">{{ item.glycosylatedHemoglobinValue }}</span
                            >％ <br />
                            检查日期：<span class="lineBox alignCenter">{{ $formatDate(item.examDate) }}</span>
                        </td>
                    </tr>
                    <tr>
                        <td v-for="(item, index) in visitList" class="noOver">
                    ...
                    <tr>
                        <td v-for="(item, index) in visitList" class="noOver">
                            <span v-if="item.hypoglycemiaReactionCode == '1'">无</span>
                            <span v-if="item.hypoglycemiaReactionCode == '2'">偶尔</span>
                            <span v-if="item.hypoglycemiaReactionCode == '3'">
                                频繁<span class="lineBox" :title="item.drugReactionsDescribe">{{ item.drugReactionsDescribe }}</span> </span
                            >&nbsp;
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td v-for="(item, index) in visitList" style="text-align: left;">
                            <div class="divBox">
                                种类：<span class="lineBox alignCenter" :title="item.insulinDrugType">{{ item.insulinDrugType }}</span
                                ><br />
                            </div>
                            用法和用量：每日<span class="lineBox alignCenter">{{ item.insulinUseFrequency }}</span
                            >次&nbsp;&nbsp; 每次<span class="lineBox alignCenter">{{ item.insulinDose }}</span
                            >单位
                        </td>
                    </tr>
                    <tr>
                        <td v-for="(item, index) in visitList" :key="index" style="text-align: left;padding:0">
                            <div v-for="(row, val) in item.drugArr" class="drug-box" :key="val" :class="{ 'last-drug-box': val == item.drugArr.length - 1 }">
                                <template v-if="row.drugName">
                                    <div class="divBox">
                                        药物名称：
                                        <el-popover placement="top-start" width="200" trigger="hover" :content="row.drugName">
                                            <span class="lineBox" slot="reference">{{ row.drugName }}</span>
                    ...
                    <!-- <tr>
						<td class="mark" v-for="(item, index) in visitList">{{ item.drugName }}&nbsp;</td>
					</tr>
                    <tr>
                        <td v-for="(item, index) in visitList">
                            每日<span class="lineBox alignCenter">{{ item.drugUseFrequency }}</span
                            >次&nbsp;<br />
                            每次<span class="lineBox alignCenter">{{ item.drugDoseage }}{{ $aadata.drugDoseageUnit[item.drugDoseageUnit + ''] }}</span>
                        </td>
                    </tr> -->
                    <tr>
...
export default {
    ...
    created() {
        ...
        // this.report = report.PersonalType2DiabetesVisit || {};
        // // console.log(this.reportData)
        // if(report){
        //     this.formatList(report.PersonalDiabetesMedicines);
        // }
        var report = this.reportData
        if (report) {
    ...
    methods: {
        ...
        /* formatList(drugList) {
            ...
            if (detailList) {
                if (detailList.length > this.medNormalLength) {
                    list = detailList;
                } else {
                    var CL = this.medNormalLength - detailList.length;
                    list = detailList;
                    for (var i = 0; i < CL; i++) {
                        list.push({});
                    }
                ...
                for (var i = 0; i < this.medNormalLength; i++) {
                    list.push({});
                }
            }
            this.medList = list;
